                  ...
                  <div className="grid grid-cols-3 sm:grid-cols-5 lg:grid-cols-9 gap-8 md:gap-12">
                    {pubStats.map((stat, index) => {
                      const getStatColor = (label: string) => {
                        switch(label) {
                          case 'SCIE': case 'SSCI': case 'A&HCI': return '#D6B14D'
                          case 'ESCI': case 'Scopus': return '#D6C360'
                          case 'Other Int\'l': return '#9A7D1F'
                          case 'Int\'l Conf': return '#AC0E0E'
                          case 'Dom. Conf': return '#E8889C'
                          case 'KCI': return '#64748b'
                          default: return '#D6B14D'
                        }
                      }
                      ...
                      <div key={index} className="group relative bg-white border border-gray-100 rounded-2xl p-12 md:p-16 hover:shadow-lg transition-all duration-300"
                        onMouseEnter={(e) => { e.currentTarget.style.borderColor = color + '66'; e.currentTarget.style.boxShadow = '0 10px 15px -3px ' + color + '1a' }}
                        onMouseLeave={(e) => { e.currentTarget.style.borderColor = '#f3f4f6'; e.currentTarget.style.boxShadow = 'none' }}
                      >
                        <div className="absolute top-0 left-12 right-12 h-[2px] rounded-full opacity-0 group-hover:opacity-100 transition-opacity" style={{background: `linear-gradient(to right, ${color}99, transparent)`}} />
                        <div className="flex flex-col items-center text-center">
                          <span className="text-lg md:text-xl font-bold mb-2" style={{color}}>{stat.count}</span>
